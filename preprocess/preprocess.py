...
def _nprint_files(input_file, output_file, flag):
    if flag == 0:
        cmd1 = 'nprint -P ' + input_file + ' --relative_timestamps  --tcp --udp --ipv4 --icmp -W ' + output_file
        ...
        cmd1 = 'nprint -P ' + input_file + ' --relative_timestamps  --tcp --udp --ipv4 --icmp -p 20 -W ' + output_file
        subprocess.call(cmd1, shell=True)
        # cmd2 = 'mv ' + input_file + ' ' + path
        # subprocess.run(cmd2, shell = True)
    # elif
...
def _time_series(input_file, output_file):
    try:
        with open(output_file, "w") as outfile:
            subprocess.run(
            ["tshark", "-r", input_file, "-T", "fields", "-E", "separator=/t","-e", "frame.time_relative","-e","tcp.flags","-e", "frame.cap_len", "-e", "ip.src", "-e", "tcp.srcport", "-e", "udp.srcport", "-e", "ip.dst", "-e", "tcp.dstport","-e", "udp.dstport", "-e", "ip.proto", "-e", "tls.handshake.extensions_server_name"], stdout=outfile, check=True)
    except:
...
def pcap_split(input_dir, output_dir):
    arg_list = []
    for file in os.listdir(input_dir):
        #if file.endswith('.pcap'):
        lst = (input_dir + '/' + file, output_dir)
        arg_list.append(lst)
...
def _patator_traffic_split(input_file, attack_folder, benign_folder):
    x = subprocess.run(["tshark", "-r", input_file, "-T", "fields", "-E", "separator=/t", "-e", "ip.src", "-e", "ip.dst"], check=True, capture_output = True)
    if any([k.strip() in ["169.231.8.224", "169.231.208.176", "169.231.9.149", "169.231.124.86", "169.231.54.0", "169.231.86.147", "169.231.111.207", "169.231.178.136", "169.231.29.156", "169.231.147.74", "169.231.216.129", "169.231.180.92"] for k in flatten([j.split("\t") for j in [i for i in x.stdout.decode().strip().split("\n")]])]):
...
if __name__ == '__main__':
    ...
    #nprint_conversion("Friday-02-03-2018/pcap_split", "Friday-02-03-2018/pcap_npts", 0)
    ...
    # nprint_conversion("/data/patator-multi-cloud-pcaps/1","/data/patator-multi-cloud-npts/1")
    # nprint_conversion("/data/patator-pcaps/1", "/data/patator-npts/1")
    print(count)
    pass